...
| B16 | 🔧 | Sauvegarde continue (PITR) sur les tables DynamoDB | ouvert | Suggestion Claude, 10/08/2026 — **vérifié : PITR désactivé sur `liensculturels-members`, `-cotisations` et `-depenses`**. Ces tables contiennent maintenant des données réelles (membres, paiements, finances) sans aucune protection contre une suppression accidentelle — plusieurs suppressions manuelles ont été faites cette session (nettoyage de données de test), toujours vérifiées avant coup mais le filet de sécurité n'existe pas si une erreur survenait. Activation simple (`aws dynamodb update-continuous-backups`), coût marginal. |
| B17 | 🔧 | Alarmes CloudWatch sur les erreurs des Lambdas (surtout paiement) | ouvert | Suggestion Claude, 10/08/2026 — **vérifié : aucune alarme CloudWatch configurée pour aucune des 8 Lambdas**. Tous les incidents de cette session (webhook mal configuré, doublon de paiement) ont été découverts parce qu'un humain a remarqué un problème, jamais par une alerte automatique. Une alarme simple sur le taux d'erreur de `liensCulturels-payment` (paiements réels désormais) enverrait un e-mail dès qu'un paiement échoue silencieusement, au lieu d'attendre qu'un membre signale le problème. |
| B18 | 🔧 | Protection anti-abus sur les formulaires publics (adhésion, contact, newsletter) | ouvert | Suggestion Claude, 10/08/2026 — **vérifié : aucun WAF sur CloudFront, aucune limite de débit configurée sur l'API Gateway** (throttling par défaut du compte AWS uniquement, très permissif). `POST /adhesion` crée un vrai compte Cognito et envoie 2 e-mails à chaque soumission, sans aucune limite — une attaque automatisée pourrait créer de nombreux faux comptes et épuiser le quota d'envoi SES. Un throttling raisonnable sur ces 3 routes (`aws apigatewayv2 update-stage` avec des `ThrottleSettings` par route) serait une protection simple et gratuite. |
| B19 | ✨ | Rappel automatique de renouvellement de cotisation | ouvert | Suggestion Claude, 10/08/2026. Les cotisations sont annuelles (champ `annee` déjà en base) mais rien ne prévient un membre que la sienne arrive à échéance — le renouvellement dépend entièrement de son initiative. Une Lambda planifiée (EventBridge, une fois par jour ou par semaine) qui envoie un e-mail de rappel X jours avant l'échéance réduirait probablement les impayés "par oubli" plutôt que par refus. |
