...
| B7 | ✨ | Trombinoscope des membres dans l'espace membre | ouvert | Demandé le 10/08/2026 pour la prochaine version. Nécessite que les membres aient renseigné une photo de profil (déjà possible depuis l'espace membre) — probable prérequis : décider si le trombinoscope liste tous les membres ou seulement le bureau, et sa visibilité (tous les membres connectés, ou juste admin/gouvernance). |
...
| B10 | 🐛 | Pack famille : le paiement depuis l'espace membre ne permet pas d'ajouter les membres de la famille (contrairement à la fiche d'adhésion) | ouvert | Signalé le 10/08/2026. `adhesion.html` a les lignes dynamiques d'ajout des membres de la famille ; la section paiement de `espace-membre.html` ne propose que le choix du type de cotisation (individuel/famille) sans ce détail — un membre existant qui veut payer un pack famille depuis son espace ne peut pas déclarer qui sont les autres membres. |
...
| B12 | ✨ | Déplacer le contenu de "Vie Associative" (comptes-rendus, etc.) vers l'espace membre | ouvert | Demandé le 10/08/2026 pour la prochaine version. Actuellement `vie-associative.html` est public (texte des réunions visible sans connexion, choix confirmé au round 2 — voir item 14) ; nouvelle demande de basculer ce contenu dans l'espace membre à la place. Implique de revoir ce choix de visibilité publique déjà en place. |
| B13 | ✨ | Nouveau rôle Communication (réseaux sociaux, blog, newsletter) | ouvert | Demandé le 10/08/2026 pour la prochaine version, à affecter à Ronel et Christiane. Contrairement à Gouvernance (volontairement sans bypass admin), **l'administrateur doit avoir accès à ce nouveau rôle** — comportement à l'opposé de Gouvernance, à bien distinguer à l'implémentation (nouveau groupe Cognito `communication`, avec bypass `admin` cette fois, contrairement au pattern déjà en place pour `gouvernance`). Périmètre exact des outils (RS, blog, newsletter — la newsletter existe déjà côté admin, à voir si elle doit migrer vers ce nouveau rôle ou rester partagée) à définir avant implémentation. |
| B14 | ✨ | Certificat de paiement téléchargeable depuis l'espace membre | ouvert | Demandé le 10/08/2026. Après un paiement (cotisation/don), un certificat PDF attestant du paiement (logo et informations de l'association, signé Trésorier + Président) serait généré et téléchargeable à volonté via un bouton dans l'espace membre. Implique : génération PDF (le dépôt `RMS/Scripts` a déjà un précédent avec `reportlab` pour d'autres documents RMS, réutilisable comme référence), nouvelle route API (génération à la demande ou stockage S3 après paiement ?), un bouton de téléchargement côté `espace-membre.html`. "Signé" s'entend vraisemblablement comme nom/fonction imprimés, pas une signature électronique cryptographique — à confirmer avant de cadrer le développement. |
| B15 | 🔧 | Suite de tests automatisés minimale (smoke tests en CI) | ouvert | Suggestion Claude, 10/08/2026. Le site n'a aucun test automatisé — chaque bug réel trouvé cette session (groupes Cognito espace-vs-virgule, webhook non idempotent, doublon par e-mail différent...) l'a été manuellement, après coup, jamais avant mise en production. Un petit ensemble de tests Playwright (connexion, soumission d'adhésion, un paiement en mode test si Stripe repasse en test, chargement des pages internes) exécuté automatiquement dans `.github/workflows/` à chaque déploiement attraperait une régression basique avant qu'un vrai utilisateur ne la découvre. |
