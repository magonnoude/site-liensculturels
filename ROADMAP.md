...
| B22 | ✨ | Plateforme V2.0 — rubrique "Escales Jumelles" (découverte territoriale) + pistes complémentaires | ouvert, figé côté nom/structure — implémentation non démarrée | Issu du compte-rendu de réunion du 12/08/2026 (Modeste, Deen, Gaëlle, Franck Olivier), affiné le 16/08/2026. **Nom retenu : "Escales Jumelles"** (préféré à "D'ici et d'ailleurs", jugé trop générique/déjà utilisé en France). **4 sous-rubriques figées** (regroupées depuis une première liste de 7, jugée trop fine) : 1) *Lieux & Paysages* (patrimoine bâti + nature), 2) *Savoir-faire & Saveurs* (artisanat + gastronomie + vie agricole/économique), 3) *Vivre & Célébrer* (traditions, musique, événements), 4) *Visages du Jumelage* (portraits/témoignages membres, volontairement distincte des 3 autres — contenu communautaire, pas éditorial). S'applique aux 3 lieux : Savè, Nogent-l'Artaud, région des Caraïbes. Intégré au plan d'affaires le 13/08/2026. **Implémentation proposée le 16/08/2026, révisée le même jour** : demande explicite de l'utilisateur d'un module de gestion **séparé de Communication** — les personnes en charge des fiches Escales Jumelles ne sont pas forcément celles en charge de la communication. **Nouveau rôle "Escales" figé** (nom choisi pour suivre le même schéma que les rôles existants — Admin/Secrétariat/Trésorerie/Gouvernance/Communication — un nom de domaine fonctionnel, pas un titre de poste). Architecture révisée : nouvelle table DynamoDB `liensculturels-decouverte` (fiches lieu×rubrique, photos via upload S3 présigné, même pattern que la médiathèque), nouvelle Lambda isolée `liensCulturels-escales-api` (au lieu d'étendre communication-api), nouveau groupe Cognito `escales`, nouvelle page publique `escales-jumelles.html` + nouvelle page admin `escales.html`. Reste à trancher à l'implémentation : bypass admin autorisé (comme Communication) ou non (comme Gouvernance) — pas encore décidé. Pas encore construit — à valider en Plan Mode avant tout code (nouvelle table + nouvelle Lambda + nouveau groupe Cognito). **Pistes complémentaires proposées le 13/08/2026**, à soumettre au bureau : réseau de membres géolocalisé par ville/région, espace "échanges" participatif, cartographie interactive des 3 territoires, témoignages vidéo de membres, page "partenaires institutionnels". Stade préliminaire, aucun périmètre ni budget arrêté. |
