...
| B20 | ✨ | Export de mes données personnelles en libre-service (RGPD) | ouvert | Suggestion Claude, 10/08/2026. `confidentialite.html` promet un droit d'accès et de portabilité des données, mais aujourd'hui ça passe uniquement par une demande manuelle à contact@ — pas de bouton "télécharger mes données" dans l'espace membre. Fonctionnalité proche de B14 dans son besoin (génération d'un document à la demande, téléchargement depuis l'espace membre) — pourrait être mutualisée avec elle techniquement. |
