...
- **`mentions-legales.html` (FR+EN) mis à jour** : "Directeur de la publication" — Judicaël
  Sènan BONI → **Modeste AGONNOUDÉ, en sa qualité de Président**. Ce champ était resté périmé
  depuis le changement de Président acté à l'AGE du 20/08/2026 (`bureau.html` et
  `mot-des-dirigeants.html` étaient, eux, déjà à jour). Changement non encore committé/déployé
  à ce stade — 1 fichier, contenu seulement, pas d'infra AWS.
- **B24 (compte Stripe suspendu, KYC) marqué "fait"** dans le Backlog : la déclaration du
  changement de dirigeant a été faite côté association (RNA/préfecture) et Stripe a levé le
  blocage — confirmé par l'utilisateur. Voir le Backlog pour le détail.
- **Brouillon de demande de subvention F.A.C. 2026** (Fonds Associatif Cantonal, Département de
  l'Aisne) rempli à partir des documents existants — copie de travail, jamais l'original :
  `www.liensculturels.org-administratif/Subventions/Demande subvention FAC - BROUILLON A
  COMPLETER.pdf`. Identité/SIRET/canton (Essômes-sur-Marne, vérifié via Wikipédia) et code APE
  (9499Z, vérifié via l'API officielle `recherche-entreprises.api.gouv.fr`) remplis avec
  confiance ; objet de la demande élargi aux quatre axes 2026 communiqués par l'utilisateur
  (événements locaux type Salon de l'Afrique et des Îles d'Outre-Mer, actions caritatives/bourse
  scolaire, développement associatif/recrutement, mise en place d'un local — mis à disposition
  gratuitement par la mairie de Nogent-l'Artaud, reste à équiper en matériel informatique).
...
Décidé et implémenté le même jour, en Plan Mode (nouvelle table DynamoDB, nouvelle Lambda,
nouveau groupe Cognito — au-delà du seuil ">2 fichiers/infra AWS" de ce dépôt) :

- Nom **"Escales Jumelles"** et 4 sous-rubriques figés (Lieux & Paysages, Savoir-faire &
  Saveurs, Vivre & Célébrer, Visages du Jumelage), regroupés depuis une première liste de 7
  jugée trop fine par l'utilisateur.
- Nouveau rôle Cognito **`escales`**, volontairement séparé de Communication (pas
  nécessairement les mêmes personnes) — bypass admin autorisé, décision explicite.
- Backend : `liensCulturels-escales-api` (lecture publique, écriture réservée au groupe
  `escales`/`admin`), table `liensculturels-escales`, upload photo présigné S3 (préfixe
  dédié, aucune nouvelle permission au-delà du strict nécessaire).
- Frontend : page publique `escales-jumelles.html` (3 onglets territoire, deep-link
  `?ville=`) et page admin `escales.html` (créer/éditer/supprimer une fiche).
- **Nav restructurée sur les 22 pages publiques** : "Nos Projets"/"Bourse Scolaire" regroupés
  sous "Nos Actions ▼", nouveau menu "Escales Jumelles ▼" (3 territoires, même niveau).
- Photo Guadeloupe sourcée (Wikimedia Commons, CC BY-SA — Pointe des Châteaux), aucune photo
  réelle des Caraïbes n'existait dans le dépôt jusqu'ici.
- **Bug réel trouvé et corrigé pendant la vérification, pas introduit par ce lot** : la CSP
  CloudFront bloquait `cdnjs.cloudflare.com` en `script-src` (lightbox2) — cassait déjà
  l'agrandissement photo sur la photothèque/vidéothèque publiques existantes, personne ne
  l'avait détecté. Corrigé de façon additive (voir DOCUMENTATION-TECHNIQUE.md §7).
- Vérifié en conditions réelles (Playwright, 3 comptes jetables) : accès public sans
  connexion, création/édition/suppression de fiche, bypass admin, refus pour un compte membre
  simple — comptes et données de test nettoyés après vérification.
- **Contenu réel pas encore saisi** — structure vide, prête pour la liste détaillée que
  M. Deen doit transmettre. Pistes complémentaires du 13/08/2026 (réseau géolocalisé,
  échanges participatifs, cartographie, témoignages vidéo, page partenaires) restent ouvertes,
  non traitées ici.
...
| 53 | Google Analytics (GA4) à activer + mise à jour de `confidentialite.html` | ✅ | Bannière de consentement (RGPD — GA4 ne bénéficie pas de l'exemption CNIL) ajoutée sur les 23 pages publiques : `assets/js/consent.js` charge GA4 (`G-EMTKPDTSJY`) uniquement après clic "Accepter", choix mémorisé en `localStorage`, lien "Modifier mon choix" dans `confidentialite.html` (section Cookies réécrite). **Bug d'infrastructure trouvé et corrigé pendant la vérification réelle** : le Content-Security-Policy de la Response Headers Policy CloudFront (`liensculturels-security-headers`) bloquait `googletagmanager.com` (script) puis `google-analytics.com` (beacon de mesure) — la bannière fonctionnait mais aucune donnée n'était jamais envoyée à Google. Corrigé (accord explicite de l'utilisateur avant modification de la policy CloudFront), retesté en conditions réelles : le `page_view` arrive bien sur `region1.google-analytics.com/g/collect`. Sitemap également soumis dans Search Console (propriété domaine déjà vérifiée). |
...
| B13 | ✨ | Nouveau rôle Communication (réseaux sociaux, blog, newsletter) | fait | Fait le 11/08/2026. Nouvelle Lambda isolée `liensCulturels-communication-api` (mirroring `gouvernance-api`) : mêmes routes newsletter qu'`admin-api` (lecture/envoi/suppression), même table DynamoDB, mais autorisation `admin` **OU** `communication` — bypass admin volontaire, à l'opposé exact du choix fait pour Gouvernance. `admin.html`/`admin-api` inchangés (duplication délibérée, zéro risque sur l'existant). Nouveau groupe Cognito `communication`, nouvelle page `communication.html`, section blog/RS volontairement limitée à un texte informatif (aucun CMS n'existe pour `blog.html`, 100% statique — pas question d'en fabriquer un dans ce lot). Noms exacts confirmés dans `bureau.html` : Ronel Jethem **Atindéhou** et Christelle **Le Tallec** ("Christiane" était une confusion) — comptes non créés, aucun e-mail connu (même lacune qu'au round 9), un admin pourra leur assigner le rôle dès que l'e-mail sera fourni. **Bug réel trouvé et corrigé pendant la vérification** : première version de `communication-api` oubliait le `_DecimalEncoder` (les items newsletter contiennent des `Decimal` — DynamoDB), 500 silencieux sur `GET /communication/newsletter`, corrigé et redéployé. **Testé en conditions réelles** (Playwright, 3 comptes jetables) : compte `communication` seul → accès + newsletter réelle chargée ; compte `admin` seul (sans `communication`) → bypass fonctionne ; compte `membre` seul → refusé. |
| B14 | ✨ | Certificat de paiement téléchargeable depuis l'espace membre | fait | Fait le 11/08/2026. Nouvelle Lambda `liensCulturels-payment-certificate`, route `GET /me/certificate/{cotisationId}` (JWT), vérifie que la cotisation appartient au compte connecté **et** que `type == "cotisation"` (jamais les dons — un vrai reçu fiscal Cerfa 11580 a des règles légales non vérifiées, ce document reste un justificatif interne, mention explicite "ne constitue pas un reçu fiscal"). PDF généré à la demande avec `reportlab` (logo rasterisé une fois en PNG depuis le SVG — Inkscape/Pillow, même outillage que les favicons —, identité légale reprise de `mentions-legales.html`), signé "Le Président, Judicaël Sènan BONI" / "Le Trésorier, Modeste AGONNOUDE" — noms et fonctions imprimés uniquement, jamais de signature scannée (pattern confirmé de `RMS/Scripts/gen_proforma_v2.py`). Première utilisation d'`isBase64Encoded` dans ce projet. **Piège réel rencontré et corrigé avant tout déploiement** : un premier `pip install reportlab --target .` sans contrainte de plateforme a produit des `.so` compilés `cp312` (Python local 3.12) au lieu de `cp311` (runtime Lambda) — détecté avant déploiement en inspectant les fichiers, corrigé avec `--platform manylinux2014_x86_64 --implementation cp --python-version 3.11 --only-binary=:all:`. Bouton "Certificat" par ligne de cotisation dans `espace-membre.html` (absent sur les dons). **Testé en conditions réelles** : PDF généré et vérifié visuellement, téléchargement réel depuis le navigateur, confirmation qu'un autre compte reçoit 403 sur le `cotisationId` de quelqu'un d'autre. |
| B15 | 🔧 | Suite de tests automatisés minimale (smoke tests en CI) | fait | Fait le 11/08/2026. Nouveau job `smoke-test` (`needs: deploy`) dans `deploy.yml` : Playwright réel contre le site en production après chaque déploiement — connexion (compte Cognito permanent dédié, groupe `membre` uniquement, 2 secrets GitHub `SMOKE_TEST_EMAIL`/`SMOKE_TEST_PASSWORD`), dashboard `espace-membre.html` sans erreur console, accès refusé déconnecté sur une page gated, contenu visible connecté sur `guide-utilisation.html`/`vie-associative.html`/`trombinoscope.html`. Portée **volontairement** limitée au read-only — pas de soumission d'adhésion réelle (créerait un vrai compte + e-mails à chaque push), pas de paiement réel (Stripe en mode LIVE). Un échec n'annule pas le déploiement (aucun rollback automatique n'existe) — sert d'alerte rapide. **Premier run réel en CI confirmé vert** (10/10 assertions), testé en local avant intégration. `package.json`/`tests/` exclus du sync S3 public. |
| B16 | 🔧 | Sauvegarde continue (PITR) sur les tables DynamoDB | fait | PITR activé sur les 3 tables (`liensculturels-members`, `-cotisations`, `-depenses`) le 11/08/2026 via `aws dynamodb update-continuous-backups`. Restauration possible à n'importe quel point dans les 35 derniers jours (défaut AWS) en cas de suppression/erreur accidentelle. Coût marginal (facturé au volume de données stockées en continu, pas d'impact visible pour ces tables). |
| B17 | 🔧 | Alarmes CloudWatch sur les erreurs des Lambdas (surtout paiement) | fait | Mis en place le 11/08/2026. **Découverte au passage : le projet compte 10 Lambdas `liensCulturels-*`, pas 8** — `liensCulturels-newsletter` et `liensCulturels-public-content` manquaient à la documentation, corrigé dans `DOCUMENTATION-TECHNIQUE.md`. Nouveau topic SNS `liensculturels-alerts` (abonnement e-mail `contact@liensculturels.org`, confirmé automatiquement — probablement un scanner de lien côté messagerie). Une alarme CloudWatch par Lambda sur la métrique `Errors` (seuil ≥ 1 sur 5 min) publie sur ce topic. |
| B18 | 🔧 | Protection anti-abus sur les formulaires publics (adhésion, contact, newsletter) | fait | Throttling par route ajouté le 11/08/2026 sur `POST /adhesion`, `POST /contact`, `POST /newsletter/subscribe` (2 req/s en régime établi, rafale de 5) via `aws apigatewayv2 update-stage --route-settings`. Testé en conditions réelles (rafale de 15 requêtes en parallèle) : 429 déclenché au-delà de la limite. Pas de WAF (coût/complexité disproportionnés pour la volumétrie réelle du site) — les autres routes publiques en lecture (`/agenda`, `/gallery`) restent au plafond de compte AWS par défaut. |
| B19 | ✨ | Rappel automatique de renouvellement de cotisation | fait | Nouvelle Lambda **`liensCulturels-cotisation-reminder`** (rôle IAM dédié, lecture seule sur members/cotisations + `ses:SendEmail` scopé à `contact@`), déclenchée par une règle EventBridge le **1er novembre à 8h UTC, chaque année**. Ancrage **année civile** (décision prise avec vous) : pour chaque membre avec e-mail, compare l'année de sa cotisation la plus récente à l'année en cours — envoie un rappel individuel si elle diffère, ignore les membres jamais cotisants (pas un renouvellement) et ceux déjà à jour. Un e-mail récapitulatif part aussi vers `contact@` à chaque exécution. Testé en conditions réelles le 11/08/2026 : mode dry-run (0 envoi, juste les logs) puis un vrai envoi SES avec une fiche membre/cotisation jetable (année 2025, supprimée après vérification) — `sent: 1, failed: 0` confirmé dans les logs CloudWatch. Aucune écriture sur `statutCotisation` (rappel seulement, pas de changement de statut automatique). |
| B22 | ✨ | Plateforme V2.0 — rubrique "Escales Jumelles" (découverte territoriale) | fait | Livré et vérifié en production le 16/08/2026. Nom "Escales Jumelles" et 4 sous-rubriques (Lieux & Paysages, Savoir-faire & Saveurs, Vivre & Célébrer, Visages du Jumelage) figés le même jour, pour les 3 territoires Nogent-l'Artaud/Savè/Caraïbes. Nouveau rôle Cognito `escales` dédié (bypass admin autorisé, séparé de Communication à la demande explicite de l'association), nouvelle Lambda `liensCulturels-escales-api`, nouvelle table `liensculturels-escales`, page publique `escales-jumelles.html` + page admin `escales.html`. Nav restructurée en même temps : "Nos Projets"/"Bourse Scolaire" sous un nouveau menu "Nos Actions", nouveau menu "Escales Jumelles" (3 territoires). Photo Guadeloupe sourcée (Wikimedia Commons, CC BY-SA) faute de photo réelle existante. **Contenu réel pas encore saisi** — structure vide/fonctionnelle, prête à être remplie dès que M. Deen transmet la liste détaillée par territoire. **Pistes complémentaires du 13/08/2026 restent ouvertes** (réseau géolocalisé, échanges participatifs, cartographie interactive, témoignages vidéo, page partenaires) — non traitées dans ce lot. **Bug réel trouvé et corrigé pendant la vérification** : CSP `script-src` bloquait lightbox2 (`cdnjs.cloudflare.com`) — bug pré-existant sur la photothèque/vidéothèque publiques, pas introduit par cette livraison, corrigé au passage (voir DOCUMENTATION-TECHNIQUE.md §7). Vérifié en conditions réelles (Playwright) : page publique (3 onglets, 4 rubriques, deep-links par ville), création/édition/suppression de fiche par un compte `escales`, bypass admin confirmé, accès refusé pour un compte membre simple, carte "Espace Escales Jumelles" visible/masquée selon le rôle — comptes et données de test nettoyés après vérification. |
| B20 | ✨ | Export de mes données personnelles en libre-service (RGPD) | fait | Nouvelle route `GET /me/export` sur `liensCulturels-member-profile` (aucun nouveau droit IAM — lecture déjà accordée sur members/cotisations), bouton "Télécharger mes données" dans `espace-membre.html` (téléchargement JSON côté client, `profil` + `cotisationsEtDons`). `confidentialite.html` (FR+EN) mentionne désormais ce canal libre-service, en gardant contact@ pour rectification/effacement. Testé en conditions réelles (Playwright, compte jetable) : le fichier téléchargé contient exactement le profil et l'historique de ce compte, rien d'un autre membre — nettoyé après vérification. |
| B24 | 🐛 | Compte Stripe suspendu — vérification KYC en cours | fait | Ouvert le 16/08/2026, bloquant tant que le dossier de conformité n'était pas validé (point bloquant : pièce d'identité expirée de Judicaël Sènan BONI, alors Président). **Résolu** suite au changement de Président (Modeste Agonnoudé, AGE du 20/08/2026, voir aussi le renommage du Directeur de la publication dans `mentions-legales.html` le 23/09/2026) : la déclaration du changement de dirigeant a été faite côté association (RNA/préfecture), et Stripe a levé le blocage — confirmé par l'utilisateur le 23/09/2026. Les paiements (cotisations/dons/bourse scolaire) sont de nouveau opérationnels. |
